...
(function($) {
    ...
    _isSwipe = function () {
        ...
        ret = false;
        if (CURRENT_TOUCH[0]) {
            move_horizontal = Math.abs(FIRST_TOUCH[0].x - CURRENT_TOUCH[0].x) > 100;
            move_vertical = Math.abs(FIRST_TOUCH[0].y - CURRENT_TOUCH[0].y) > 100;
        ...
        if (Math.abs(diff) > 20 || GESTURE.angle_difference !== 0) {
            ...
            while (Math.abs(diff - GESTURE.angle_difference) > 90 && i++ < 10) {
                // eval("diff " + symbol + "= 180;");
				 return Function("diff " + symbol + "= 180;")();
            }
